...
class SslCommerzPaymentController extends Controller
{
    ...
    public function payViaAjax(Request $request)
    {
        ...
        $post_data = array();
        $post_data['total_amount'] = $payload->total_cost; # You cant not pay less than 10
        $post_data['currency'] = "BDT";
        $post_data['tran_id'] = $payload->order_id; // tran_id must be unique

        # CUSTOMER INFORMATION
        $post_data['cus_name'] = $payload->name;
        $post_data['cus_email'] = $payload->email ?? auth()->user()->id;
        $post_data['cus_add1'] = 'Customer Address';
        $post_data['cus_add2'] = "";
        $post_data['cus_city'] = "";
        $post_data['cus_state'] = "";
        $post_data['cus_postcode'] = "";
        $post_data['cus_country'] = "Bangladesh";
        $post_data['cus_phone'] = $payload->mobile_number;
        $post_data['cus_fax'] = "";
    ...
    public function success(Request $request)
    {
        ...
        $order_details = DB::table('orders')
            ...
            ->select('transaction_id', 'status', 'currency', 'amount', 'service_type')->first();
            ...
            if ($validation) {
                ...
                */
                ...
                if ($order_details->service_type == 'motorcycle') {
                    ...
                    $policy = DB::table('omi_overseas_mediclaim_insurences')
                        ->where('transaction_id', $tran_id)
                        ->first();
                    return redirect()->to('omi/invoice?payment_status=' . $policy->payment_status . '&order_id=' . $tran_id . '-' . rand(1000, 9999));
                }
            }
